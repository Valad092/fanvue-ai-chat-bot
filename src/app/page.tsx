    ...
    <div className="font-sans grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20">
      <main className="flex flex-col gap-[24px] row-start-2 items-center sm:items-start max-w-[720px] w-full">
        ...
        <div className="w-full rounded-lg border border-black/[.08] dark:border-white/[.145] p-4 sm:p-6">
          ...
          <div className="mt-4">
            {isAuthed ? (
              <pre className="text-xs overflow-auto p-3 rounded bg-black/[.05] dark:bg-white/[.06]">
                {JSON.stringify(me, null, 2)}
              </pre>
            ) : (
              <p className="text-sm text-black/70 dark:text-white/70">
                Click &quot;Login with Fanvue&quot; to authenticate and we will
                display your current user from the Fanvue API.
              </p>
            )}
          </div>
        </div>
